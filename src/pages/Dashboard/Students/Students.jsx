...
const Students = () => {
      ...
      title: (
        <div>
          {data.length > 0 ? (
            ...
            <h5 className="text-center mt-5 sentence-case">
              You have not assigned in any courses.
            </h5>
          )}
        </div>
      ),
  ...
  const handleDeleteSubmit = async () => {
    try {
      ...
      await axiosWrapper(
        "DELETE",
        API_URL.DELETE_STUDENT.replace(":id", showDeleteModal?.studentId),
        {},
        token
      );
      fetchData();
    ...
    const percentages = courses.map((course) => {
      ...
      const completedLectures = lectures.filter((lecture) => {
        ...
        return (
          ...
          completedBy.some((item) => item._id === studentId)
        );
      }).length;
    ...
    <div key={props.data._id}>
      <div className="d-flex align-items-center">
        <div
          ...
          onClick={() => {
            props.onRoadMapClick(
              props.data.coursesRoadmap || [],
              props.data._id
            );
          }}
        >
  ...
  const columns = [
    ...
    //     return (
    //       <div key={rowData._id}>
    //         {coachingTrajectory === COACH.COACH_TYPE.HIGH_TICKET ? "HT" : "LT"}
    //       </div>
    //     );
    //   },
    // },
  ...
  return (
    <div className="students-page">
      <Helmet>
        <title>Students | Account Academy</title>
      </Helmet>
      {coursesModal.show && (
        <Modal
          size="md"
          show={coursesModal.show}
          onClose={handleCloseModal}
          title={coursesModal.title}
        >
          <RoadMapList
            ...
            setCoursesMap={(data) =>
              handleRoadmapUpdate(data, coursesModal.courseId)
            }
          />
        </Modal>
      )}
                  ...
                  <span>
                    {
                      studentsTrajectory.find(
                        (s) =>
                          s.value === selectedOption ||
                          s.label === selectedOption
                      ).label
                    }
                  </span>
                  <img src={downArrow} alt="Down arrow" />
