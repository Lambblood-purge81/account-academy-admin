...
const Events = () => {
  ...
  const fetchData = async (query) => {
    ...
    try {
      setLoading(true);
      const events = await axiosWrapper(
        "GET",
        `${API_URL.GET_ALL_EVENTS}?dateTime=${query || ""}`,
        {},
        token
      );
  ...
  const handleEditClick = (id) => {
    ...
    dispatch({
      ...
      data: { keyOfData: "eventId", data: id },
    });
  ...
  const handleDeleteSubmit = async () => {
    try {
      ...
      await axiosWrapper(
        "DELETE",
        API_URL.DELETE_EVENT.replace(":id", showDeleteModal?.eventId),
        {},
        token
      );
      fetchData();
  ...
  const LinkRenderer = (props) => (
    <div key={props.data._id}>
      <OverlayTrigger
        placement="top"
        overlay={<Tooltip id="tooltip-top">{props.value}</Tooltip>}
      >
  ...
  const columns = [
    ...
    },
    {
      ...
      autoHeight: true,
      valueGetter: (params) => {
        return params.data.typeOfEvent === "ONLINE"
  ...
  return (
    <div className="events-page">
      <Helmet>
        <title>Events | Account Academy</title>
      </Helmet>
      {studentModal.show && (
        <Modal
          size="large"
        ...
        children={
          <div className="btn-wrapper">
            <div className="text-nowrap filter-by-text d-flex justify-content-even align-items-center">
              ...
              >
                {["All Events", "Upcoming Events", "Past Events"].map(
                  (event) => (
                    <Dropdown.Item
            ...
            <Button className="add-button" onClick={handleCreateClick}>
              ...
              <span className="ms-1">Create New Event</span>
            </Button>
          </div>
        }
      />
