...
const StudentDashboard = () => {
  ...
  const fetchDashboardData = async (loading = true) => {
    ...
    try {
      ...
      cardData = await axiosWrapper(
        "GET",
        API_URL.GET_STUDENT_CARD_DATA,
        {},
        token
      );

      secondCard = await axiosWrapper(
        "GET",
        API_URL.GET_STUDENT_SECOND_CARD_DATA,
        {},
        token
      );
      ...
      const mapCards = Object.entries(cardData?.data).map(
        ([key, value], index) => ({
          id: index,
          title: convertCamelCaseToTitle(key),
          value: amountKeys.includes(key)
            ? `$${formatNumberWithCommas(value)}`
            : value,
        })
      );

      const mapSecondCards = Object.entries(secondCard?.data).map(
        ([key, value], index) => ({
          id: index + 1,
          title: convertCamelCaseToTitle(key),
          value: amountKeys.includes(key)
  ...
  return (
    <div className="dashboard-page">
      <Helmet>
        <title>Dashboard | Account Academy</title>
      </Helmet>
      {false ? (
