  ...
  const fetchDashboardData = async (loading = true) => {
    try {
      ...
      if (role === "ADMIN") {
        ...
        eventData = await axiosWrapper(
          "GET",
          API_URL.GET_ADMIN_EVENTS_DATA,
          {},
          token
        );
        setDataSet(true);
  ...
  const getEventDetails = async (id) => {
    try {
      const response = await axiosWrapper(
        "GET",
        API_URL.GET_EVENT.replace(":id", id),
        {},
        token
      );
