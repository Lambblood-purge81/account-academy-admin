...
const Coaches = () => {
  ...
  const handleDeleteSubmit = async () => {
    try {
      ...
      await axiosWrapper(
        "DELETE",
        API_URL.DELETE_COACH.replace(":id", showDeleteModal?.coachId),
        {},
        token
      );
      fetchData(false);
  ...
  const columns = [
    ...
    },
    {
      ...
      cellRenderer: ({ data: rowData }) => {
        ...
        return (
          <div key={rowData._id}>
            {coachType === COACH.COACH_TYPE.HIGH_TICKET ? "HT" : "LT"}
          </div>
        );
      },
    },
  ...
  return (
    <div className="coaches-page">
      <Helmet>
        <title>Coaches | Account Academy</title>
      </Helmet>
      {productModal.show && (
        <Modal
          size="large"
      ...
      <Table
        ...
        children={
          <div className="d-flex justify-content-end">
            ...
            >
              <img src={add} alt="" />{" "}
              <span className="ms-2">Add New Coach</span>
            </Button>
          </div>
        }
      />
