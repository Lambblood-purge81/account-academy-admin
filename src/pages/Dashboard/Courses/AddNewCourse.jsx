...
const AddNewCourse = () => {
  ...
  const currentCourseUpdate = useSelector(
    (state) => state?.root?.currentCourseUpdate
  );
  ...
  const getCourseById = async (id) => {
    try {
      ...
      dispatch({
        ...
        data: { keyOfData: "currentCourseUpdate", data: false },
      });
      dispatch({
        type: types.ALL_RECORDS,
        data: { keyOfData: "coachName", data: data.moduleManager?.name },
      });
    } catch (error) {
      dispatch({
        type: types.ALL_RECORDS,
  ...
  const getLectures = async (id) => {
    try {
      const { data } = await axiosWrapper(
        "GET",
        `${API_URL.GET_ALL_LECTURES}?courseId=${id}`,
        {},
        token
      );
      updateCourseData({
      ...
      dispatch({
        type: types.ALL_RECORDS,
        data: { keyOfData: "lectureUpdate", data: false },
      });
    } catch (error) {
      dispatch({
        type: types.ALL_RECORDS,
  ...
  const createOrUpdateCourse = async (formData) => {
    if (currentCourse) {
      ...
      dispatch({
        type: types.ALL_RECORDS,
        data: { keyOfData: "currentCourse", data: course?.data?._id },
      });
    }
    ...
    dispatch({
      type: types.ALL_RECORDS,
      data: { keyOfData: "currentCourseUpdate", data: true },
    });
  };
  ...
  const handlePublishCourse = async () => {
    await axiosWrapper(
      "PUT",
      `${API_URL.PUBLISH_COURSE.replace(":id", currentCourse)}`,
      {},
      token
    );
    // Call the get Course By Id so we can have updated state of part one
  ...
  useEffect(() => {
    if (courseId) {
      dispatch({
        type: types.ALL_RECORDS,
        data: { keyOfData: "currentCourse", data: courseId },
      });
