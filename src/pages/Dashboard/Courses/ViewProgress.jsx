    ...
    if (progress) {
      const debouncedFilter = debounce(() => {
        const filteredLectures = progress.lectures
          .filter((lecture) =>
            lecture.lectureTitle.toLowerCase().includes(search.toLowerCase())
          )
          .slice(0, 20);
        ...
        <div className="text-heading">
          ...
          <div className="viewProfile-img">
            <img
              ...
              alt="student profile"
            />
            <p>{progress?.student?.name || "Account Academy X"}</p>
          </div>
        </div>
      </div>
      <div className="search-lectures">
        ...
        <div className="title-lecture-btns">
          <h1>
            ...
            <span>
              {" "}
              (Note: Green labeled lectures are passed by student)
            </span>{" "}
          </h1>
        </div>
              ...
              filteredLectures?.map((lecture, index) => (
                <Col xs={12} sm={5} md={4} lg={4} xl={3} xxl={2} key={index}>
                  <Button
                    type="button"
                    className={
                      lecture.isCompleted ? "btn active " : "btn inactive"
                    }
                  >
                    <img
                      src={lecture.isCompleted ? activeIcon : inActiveIcon}
                      alt="IconLect"
                    />
        ...
        <div className="viewProgress-footer mx-auto">
          <Link to={`/${role}/courses`}>
            <Button
              className="done-btn text-center d-flex justify-content-center"
              type="button"
            >
              Done
            </Button>
